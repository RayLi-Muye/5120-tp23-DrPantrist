...
-- ================== 6) inventory_profiles (three 鈥減rofiles/slots鈥? share the same Shared) ==================
CREATE TABLE inventory_profiles (
  profile_id    UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  inventory_id  UUID NOT NULL REFERENCES inventories(inventory_id) ON DELETE CASCADE,
  profile_name  VARCHAR(60) NOT NULL,
  position      SMALLINT NOT NULL,   -- 1..3; typically 1 = creator, 2/3 added via 鈥?鈥?
  created_at    TIMESTAMPTZ DEFAULT now(),
...
-- ---------- 1) Remove 鈥渕embers 鈮?3鈥?related objects ----------
-- Triggers & functions (ignore if they don鈥檛 exist)
...
-- ---------- 2) profiles: add 鈥減rofile/slot鈥?table ----------
...
-- Ensure we have a function for 鈥渁uto-create default profile when a new inventory is created鈥?
...
COMMIT;
-------------------------
-- 鈶?Detailed ledger: insert one row for every 鈥渃onsumption/use鈥?event
CREATE TABLE IF NOT EXISTS consumption_ledger (
  ...
  -- Snapshot at write-time (future price/factor changes won鈥檛 affect history)
...
-- Generated columns: persist 鈥渕oney/CO鈧?saved鈥?for faster aggregations
